...
def create_app(flask_config_name=None, **kwargs):
    ...
    if not env_flask_config_name and flask_config_name is None:
        ...
        if env_flask_config_name:
            assert env_flask_config_name == flask_config_name, (
                "FLASK_CONFIG environment variable (\"%s\") and "
                "flask_config_name argument (\"%s\") are both set "
    ...
    try:
        ...
        if flask_config_name == 'local':
            app.logger.error(
                "You have to have `local_config.py` or "
                "`local_config/__init__.py` in order to use the default "
                "'local' Flask Config. Alternatively, you may set "
                "`FLASK_CONFIG` environment variable to one of the following "
