    ...
    def validate_user_id(self, data):
        if current_user.id != data:
            raise ValidationError(
                "It is only allowed to query your own OAuth2 clients.")
...
class CreateOAuth2ClientParameters(PostFormParameters):
    ...
    def validate_default_scopes(self, data):
        unknown_scopes = set(data) - \
            set(api.api_v1.authorizations['oauth2_password']['scopes'])
        if unknown_scopes:
            raise ValidationError(
                "'%s' scope(s) are not supported." %
                (', '.join(unknown_scopes))
            )
