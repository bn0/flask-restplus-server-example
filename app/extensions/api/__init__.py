...
api_v1 = Api(  # pylint: disable=invalid-name
    ...
    description=(
        "It is a [real-life example RESTful API server implementation using "
        "Flask-RESTplus] "
        "(https://github.com/frol/flask-restplus-server-example).\n\n"
        "This demo features:\n"
        "* Self-documented RESTful API server using autogenerated OpenAPI "
        "specifications;\n"
        ...
        "* 95+% code coverage.\n\n"
        "## Explore the demo\n\n"
        "I suggest you start with signing up a new user. To do so, use "
        "`POST /users/` endpoint "
        "with `recaptcha_key=\"secret_key\"`.\n\n"
        "You will need to know the API Client ID to authenticate, so "
        "here it is: `documentation`. Sometimes (e.g. for token refreshing) "
        "you might need API Client Secret: "
...
def serve_swaggerui_assets(path):
    ...
    if not current_app.debug:
        import warnings
        warnings.warn(
            "/swaggerui/ is recommended to be served by public-facing server "
            "(e.g. NGINX)"
        )
